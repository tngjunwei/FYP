...
class Model:
  ...
  def encode_obs(self, obs):
    ...
    logvar = logvar[0]
    s = logvar.shape
    logvar[logvar > 10.0] = 0.0
    z = mu + np.exp(logvar/2.0) * np.random.randn(*s)
    return z, mu, logvar

  def onehot(self, a):
    a = a.astype('int32')
    tmp = np.zeros((self.output_size,1))
    tmp[a] = 1
    return tmp
  ...
  def get_action(self, z):
    ...
    self.state = rnn_next_state(self.rnn, z, self.onehot(action), self.state)
  ...
  def set_model_params(self, model_params):
    if EXP_MODE == MODE_Z_HIDDEN: # one hidden layer
      ...
      self.weight_output = params_2[self.output_size:].reshape(self.hidden_size, 3)
    else:
