...
from rnn.psrnn import PSRNN, Config
from rnn.two_stage_regression import get_params
from rnn.data_producer import DataProducer
...
s = data_logvar.shape
data = data_mu + np.exp(data_logvar/2.0) * np.random.randn(*s)
#max_seq_len = hps_model.max_seq_len
max_seq_len = config.num_steps

N_data = len(data) # should be 10k
#batch_size = hps_model.batch_size
batch_size = config.batch_size

data_producer = DataProducer(data, max_seq_len, batch_size)
...
params = get_params(data.reshape((-1, config.vocab_size)), config, no_init=True)
rnn = PSRNN(params, config, is_training=True)
rnn.load_json(no_init=True)
...
for epoch in range(config.max_epoch):
  data_producer.reset()
  for local_step in range(data_producer.get_epoch_size()):
    ...
    curr_learning_rate = config.learning_rate
    #raw_z, raw_a = random_batch()
    inputs, outputs = data_producer.next()
    ...
    if (step%20==0 and step > 0):
      ...
      start = time.time()
      output_log = "epoch: %d, step: %d, lr: %.6f, cost: %.4f, train_time_taken: %.4f" % (epoch, step, curr_learning_rate, train_cost, time_taken)
      print(output_log)
