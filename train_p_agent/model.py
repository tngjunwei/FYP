...
from rnn.psrnn import PSRNN, rnn_init_state, rnn_next_state, rnn_output, rnn_output_size, Config
...
class Model:
  ...
  def __init__(self, load_model=True):
    ...
    self.env_name = 'pong'
    self.z_size = 16 #32
    self.vae = ConvVAE(z_size=self.z_size, batch_size=1, gpu_mode=False, is_training=False, reuse=True)
    self.vae.load_json('vae/vae.json')
    print("Done")

    #self.rnn = MDNRNN(hps_sample, gpu_mode=False, reuse=True)
    config = Config()
    params = get_params(None, config)
    self.rnn = PSRNN(params, config, is_training=False, gpu_mode=False, reuse=True)
    if load_model:
    ...
    self.output_size = 6  # 3 for carracing
    ...
    if EXP_MODE == MODE_Z_HIDDEN: # one hidden layer
      ...
      self.param_count = ((self.input_size+1)*self.hidden_size) + (self.hidden_size*self.output_size+self.output_size)
    else:
      self.weight = np.random.randn(self.input_size, self.output_size)  # is 3 the output size?
  ...
  def get_action(self, z):
    ...
    if EXP_MODE == MODE_Z_HIDDEN: # one hidden layer
      ...
      #action = np.tanh(np.dot(h, self.weight) + self.bias) # (vocab_size) @ (vocab_size x num_outputs) = (num_outputs)
      action = np.argmax(softmax((np.dot(h, self.weight) + self.bias)))
...
def simulate(model, train_mode=False, render_mode=False, num_episode=5, seed=-1, max_len=-1):
  ...
  for episode in range(num_episode):
    ...
    state = np.zeros((num_steps,Config.vocab_size))
    ...
    for t in range(max_episode_length):
      ...
      z = np.reshape(z, (-1, Config.vocab_size))
      if t < num_steps:
        state = np.concatenate([state[:num_steps-t-1], state[num_steps-t:], z], axis=0)
      else:
        state = np.concatenate([state[1:], z], axis=0)

      #action = model.get_action(z)
      action = model.get_action(state)
      recording_z.append(z.reshape(-1))
    ...
    recording_z = np.array(recording_z, dtype=np.float16)
  ...
  np.savetxt("action.txt", recording_action)
  np.savetxt("z.txt", recording_z)
  np.savetxt("mu.txt", recording_mu)

  # print(recording_action.shape)
  # print(recording_z.shape)
  # print(recording_mu.shape)
